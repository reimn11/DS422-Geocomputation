...
# Copy it three time for baseline, storm, recovery
baseline_dates <- seq.Date(from = as.Date("2025-01-15"), to = as.Date("2025-01-24"), by = "day")

all_dates <- c(baseline_dates, storm_dates) # add recovey_dates
```
...
completeness_check <- tibble(
  date = c(baseline_ok, storm_ok), #add recovery
...
ggplot(statewide_trend, aes(x = date, y = ntl_mean, fill = period)) +
  geom_col() +
  scale_fill_manual(values = c("Baseline" = "grey70", "Storm" = "firebrick")) + # we need to change the date 
...
kilauea_dates <- ntl_trend |>
  filter(county == "Hawaii", period == "Baseline", date >= as.Date("2025-01-15")) |> # change date to 2026-08-03 
  pull(date)
