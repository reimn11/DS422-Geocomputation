...
baseline_dates <- seq.Date(from = as.Date("2026-08-03"),
                           to   = as.Date("2026-08-13"), by = "day")

storm_dates <- seq.Date(from = as.Date("2026-08-14"),
                           to   = as.Date("2026-08-16"), by = "day")

recovery_dates <- seq.Date(from = as.Date("2026-08-17"),
                           to   = as.Date("2026-08-21"), by = "day")
...
ggplot(statewide_trend, aes(x = date, y = ntl_mean, fill = period)) +
  ...
  labs(title = "Statewide Mean Nighttime Radiance by Day",
       subtitle = paste0("Baseline (Aug 3-13) vs. Storm Lala (Aug 14-16) vs. Recovery (Aug 17-21), 2026 (", storm_date_label, ")"),
...
kilauea_dates <- ntl_trend |>
  filter(county == "Hawaii", period == "Baseline", date >= as.Date(c("2026-08-13", "2026-08-14"))) |> # change date to 2026-08-03 
  pull(date)
...
paired_values <- tibble(
  ...
  storm = values(storm_stack)[, 1],
  recovery = values(recovery_stack)[, 1]
) |>
...
ggplot(paired_values, aes(x = baseline, y = recovery)) + #
  geom_point(alpha = 0.15, size = 0.5) +
  geom_abline(slope = 1, intercept = 0, color = "blue", linetype = "dashed") +
  labs(title = paste0("Oahu: After Storm vs. Baseline Radiance, Per Pixel (", storm_date_label, ")"),
       subtitle = "Points below the dashed line got darker; above got brighter",
       x = "Baseline radiance", y = "After storm radiance") +
  theme_minimal()

ggplot(paired_values, aes(x = recovery, y = storm)) + #
  geom_point(alpha = 0.15, size = 0.5) +
  geom_abline(slope = 1, intercept = 0, color = "green", linetype = "dashed") +
  labs(title = paste0("Oahu: Storm vs. After Storm Radiance, Per Pixel (", storm_date_label, ")"),
       subtitle = "Points below the dashed line got darker; above got brighter",
       x = "After Storm radiance", y = "Storm-night radiance") +
  theme_minimal()
```
